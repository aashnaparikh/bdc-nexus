...
var warningShown2 = /* @__PURE__ */ new Set();
var reportedErrors2 = /* @__PURE__ */ new Set();
...
var _hasLoader = (packageName, localeId) => {
  const bundleKey = `${packageName}/${localeId}`;
  return loaders3.has(bundleKey);
};
var _loadMessageBundleOnce = (packageName, localeId) => {
  const bundleKey = `${packageName}/${localeId}`;
  const loadMessageBundle = loaders3.get(bundleKey);
  if (loadMessageBundle && !bundlePromises.get(bundleKey)) {
    bundlePromises.set(bundleKey, loadMessageBundle(localeId));
  }
  return bundlePromises.get(bundleKey);
};
var _showAssetsWarningOnce2 = (packageName) => {
  if (!warningShown2.has(packageName)) {
    console.warn(
...
var fetchI18nBundle = async (packageName) => {
  ...
  if (!_hasLoader(packageName, localeId)) {
    _showAssetsWarningOnce2(packageName);
    return;
  }
  try {
    ...
    const e = error;
    if (!reportedErrors2.has(e.message)) {
      reportedErrors2.add(e.message);
      console.error(e.message);
    }
  }
};
...
//# sourceMappingURL=chunk-KRR24QQX.js.map
